...
const install = (accountConfig, featureID, eventID) => {
  return new Promise((resolve, reject) => {
    ...
    requestAccessToken(accountConfig.username, accountConfig.refresh_token).then((access_token) => {
      ...
      fetch(installURL, { body: JSON.stringify(json), method: 'POST', headers: { 'Authorization': auth, 'Content-type': 'application/json' } })
        .then((response) => {
  ...
  Promise.resolve(args)
    ...
    .then((parsed) => {
      if (!fileExists('bazaar.json')) {
        ...
        return new Promise((resolve, reject) => {
          install(configJSON, bazaarJSON.id, parsed.eventID).then((result) => {
